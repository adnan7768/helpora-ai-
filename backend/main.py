...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Restrict to your frontend URL after deploying
...
client = OpenAI(
    ...
    api_key=OPENROUTER_API_KEY or "your_openrouter_api_key_here",
)
...
async def chat(request: ChatRequest):
    ...
    if not OPENROUTER_API_KEY or OPENROUTER_API_KEY == "your_openrouter_api_key_here":
        raise HTTPException(
            status_code=500,
            detail="OPENROUTER_API_KEY is missing. Please set your OpenRouter API key in backend/.env file."
        )
    ...
    if not reply:
        raise HTTPException(status_code=500, detail=f"OpenRouter API error: {str(last_error)}")
